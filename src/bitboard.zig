...
pub const Position = struct {
    ...
    pub fn loadFen(fen: []const u8) !Position {
        ...
        pos.zobrist_hash = zobrist.computeHash(&pos);
        return pos;
    }
    ...
    pub fn toFen(self: *const Position, buffer: []u8) ![]const u8 {
        ...
        while (rank >= 0) : (rank -= 1) {
            ...
            var f: i8 = 0;
            while (f < 8) : (f += 1) {
                const sq = @as(u6, @intCast(rank * 8 + f));
                const mask = @as(u64, 1) << sq;
            ...
            if (empty_count > 0) try w.print("{d}", .{empty_count});
            if (rank > 0) try w.print("/", .{});
        }
        ...
        if (self.en_passant_sq) |sq| {
            ...
            const ep_f = @as(u8, @intCast(sq_val % 8));
            const ep_r = @as(u8, @intCast(sq_val / 8));
            try w.print("{c}{c}", .{ 'a' + ep_f, '1' + ep_r });
        } else {
            try w.print("-", .{});
        }
